...
export class ExcelService {
     ...
     */
    static exportToExcel(data, filename = 'backup-sistema-financeiro') {
        try {
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        if (workbook.SheetNames.includes('Transações')) {
                            ...
                            const transactionsJson = XLSX.utils.sheet_to_json(transactionsSheet, { header: 1 });
                            if (transactionsJson.length > 1) {
                                importedData.transactions = transactionsJson.slice(1).map((row, index) => ({
                                    ...
                                    description: row[2] || '',
                                    type: (row[3] === 'Receita' ? 'income' : 'expense'),
                                    category: row[4] || '',
                        ...
                        if (workbook.SheetNames.includes('Clientes')) {
                            ...
                            const clientsJson = XLSX.utils.sheet_to_json(clientsSheet, { header: 1 });
     ...
     */
    static getRevenueByCategory(transactions) {
        return transactions
     ...
     */
    static getExpensesByCategory(transactions) {
        return transactions
     ...
     */
    static getRevenueByClient(transactions, clients) {
        const clientStats = {};
